...
bool VL53L0X::init(bool io_2v8)
{
	...
	writeReg(0xFF, 0x01);
	writeReg(0x00, 0x00);
	stop_variable = readReg(0x91); //Får 3C herfra
	writeReg(0x00, 0x01);
	writeReg(0xFF, 0x00);
	writeReg(0x80, 0x00);
...
{	
	...
	bus->Write((address << 1) | 0); // Skriv adresse + WRITE
	bus->Write(reg); // Skriv data
	bus->Write(value);
...
uint8_t VL53L0X::readReg(uint8_t reg)
{
	...
	bus->ReadMultipleFromAddress(address, reg, &value, 1);
...
uint16_t VL53L0X::readReg16Bit(uint8_t reg)
{
	...
	bus->ReadMultipleFromAddress(address, reg, buffer, 2);
...
uint32_t VL53L0X::readReg32Bit(uint8_t reg)
{
	...
	bus->ReadMultipleFromAddress(address, reg, buffer, 4);
...
void VL53L0X::writeMulti(uint8_t reg, uint8_t const * src, uint8_t count)
{
	...
	bus->WriteMultipleToAddress(address, reg, src, count);
	//Evt flytt stop hit så vi kan oppdatere last_status
}
...
void VL53L0X::readMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
	...
	bus->ReadMultipleFromRegister(address, reg, dst, count);
