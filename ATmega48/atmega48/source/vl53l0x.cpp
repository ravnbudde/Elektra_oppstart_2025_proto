...
bool VL53L0X::init(bool io_2v8)
{
	...
	if (ReadRegister(IDENTIFICATION_MODEL_ID) != 0xEE) { return false; }
	...
	stop_variable = ReadRegister(0x91); //Får 3C herfra
...
//NB!!! på alle disse funksjonene er jeg usikker på om man bør starte hele funksjonen på nytt om det ikke er succes (eks ikke ack)
//Eller om det holder og f.eks. bare sende register på nytt
void VL53L0X::WriteRegister(uint8_t reg, uint8_t val)
{
	...
	error_code = tw_write_sla(TW_SLA_W(address));
	if (error_code != SUCCESS)
	{
		// NB!!!! litt usiker (herfra og ned) om man bare kan sende delen på nytt eller om man må sende stop og starte hele funksjonen på nytt!!
		error_code = tw_write_sla(TW_SLA_W(address));
	}
	
	/* Send data byte in single or burst mode */
	
	error_code = tw_write(reg);
	if (error_code != SUCCESS)
	{
		error_code = tw_write(reg);
	}
	
	error_code = tw_write(val);
	if (error_code != SUCCESS)
	{
		error_code = tw_write(val);
	}
...
void VL53L0X::WriteRegister16bit(uint8_t reg, uint16_t val)
{
	ret_code_t error_code;
	...
	error_code = tw_write((uint8_t)(val>>8));
	if (error_code != SUCCESS)
	{
		error_code = tw_write((uint8_t)(val>>8));
	}
	
	error_code = tw_write((uint8_t)val);
	if (error_code != SUCCESS)
	{
		error_code = tw_write((uint8_t)val);
	}
...
void VL53L0X::WriteRegister32bit(uint8_t reg, uint32_t val)
{
	ret_code_t error_code;
	...
	error_code = tw_write((uint8_t)(val>>24));
	if (error_code != SUCCESS)
	{
		error_code = tw_write((uint8_t)(val>>24));
	}
	
	error_code = tw_write((uint8_t)(val>>16));
	if (error_code != SUCCESS)
	{
		error_code = tw_write((uint8_t)(val>>16));
	}
...
uint8_t VL53L0X::ReadRegister(uint8_t reg)
{
	uint8_t value;
	ret_code_t error_code;
	error_code = tw_master_transmit(address, &reg, 1, true); //Sender adresse(w) + register uten stop-condition
	if (error_code != SUCCESS)
	{
		error_code = tw_master_transmit(address, &reg, 1, true);
	}
	error_code = tw_master_receive(address, &value, 1); //Sender start + adresse(r) og leser byte til value
	if (error_code != SUCCESS)
	{
		error_code = tw_master_receive(address, &value, 1);	
	}
...
uint16_t VL53L0X::ReadRegister16bit(uint8_t reg)
{
	...
	error_code = tw_master_receive(address, buffer, 2); //Sender start + adresse(r) og leser 2 bytes byte til buffer
	if (error_code != SUCCESS)
	{
		error_code = tw_master_receive(address, buffer, 2);
	}
...
uint32_t VL53L0X::ReadRegister32bit(uint8_t reg)
{
	...
	error_code = tw_master_receive(address, buffer, 4); //Sender start + adresse(r) og leser 4 bytes byte til buffer
	if (error_code != SUCCESS)
	{
		error_code = tw_master_receive(address, buffer, 4);
	}
...
{
	ret_code_t error_code;
	error_code = tw_master_transmit(address, src, count, false);
	if (error_code != SUCCESS)
	{
		error_code = tw_master_transmit(address, src, count, false);
	}
...
void VL53L0X::ReadMulti(uint8_t reg, uint8_t * dst, uint8_t count)
{
	...
	error_code = tw_master_receive(address, dst, count); //Sender start + adresse(r) og leser bytes til dst
	if (error_code != SUCCESS)
	{
		error_code = tw_master_receive(address, dst, count);
	}
}
...
bool VL53L0X::setSignalRateLimit(float limit_Mcps)
{
	...
	WriteRegister16bit(FINAL_RANGE_CONFIG_MIN_COUNT_RATE_RTN_LIMIT, limit_Mcps * (1 << 7));
	return true;
}
...
{
	return (float)ReadRegister16bit(FINAL_RANGE_CONFIG_MIN_COUNT_RATE_RTN_LIMIT) / (1 << 7);
}
...
bool VL53L0X::setMeasurementTimingBudget(uint32_t budget_us)
{
	...
	if (enables.final_range)
	{
		...
		WriteRegister16bit(FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI,
...
bool VL53L0X::setVcselPulsePeriod(vcselPeriodType type, uint8_t period_pclks)
{
	...
	if (type == VcselPeriodPreRange)
	{
		...
		WriteRegister16bit(PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI,
	...
	uint8_t sequence_config = ReadRegister(SYSTEM_SEQUENCE_CONFIG);
	WriteRegister(SYSTEM_SEQUENCE_CONFIG, 0x02);
...
void VL53L0X::startContinuous(uint32_t period_ms)
{
	...
	if (period_ms != 0)
	{
		...
		uint16_t osc_calibrate_val = ReadRegister16bit(OSC_CALIBRATE_VAL);
...
uint16_t VL53L0X::readRangeContinuousMillimeters()
{
	...
	uint16_t range = ReadRegister16bit(RESULT_RANGE_STATUS + 10);
...
uint16_t VL53L0X::readRangeSingleMillimeters()
{
	...
	startTimeout();
	while (ReadRegister(SYSRANGE_START) & 0x01)
	{
		if (checkTimeoutExpired())
		{
			did_timeout = true;
...
bool VL53L0X::getSpadInfo(uint8_t * count, bool * type_is_aperture)
{
	...
	startTimeout();
	while (WriteRegister(0x83) == 0x00)
	{
		if (checkTimeoutExpired()) { return false; }
	}
	WriteRegister(0x83, 0x01);
	tmp = WriteRegister(0x92);
...
void VL53L0X::getSequenceStepTimeouts(SequenceStepEnables const * enables, SequenceStepTimeouts * timeouts)
{
	...
	decodeTimeout(ReadRegister16bit(PRE_RANGE_CONFIG_TIMEOUT_MACROP_HI));
	...
	decodeTimeout(ReadRegister16bit(FINAL_RANGE_CONFIG_TIMEOUT_MACROP_HI));
